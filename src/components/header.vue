...
<template>
	<div class="">
		...
		<div class="headImgBox">
			...
			<video id="videoElement" :autoplay="isVideoPlaying" ref="videoPlayer" autoplay loop preload="auto"
				class="video-background">
				<source src="../../static/img/jay2.mp4" type="video/mp4">
			</video>
			...
			<div class="h-information">
				...
				<img :src="this.$store.state.themeObj.head_portrait ? this.$store.state.themeObj.head_portrait : 'static/img/tou.jpg'"
					alt="">
				<br />
				<h2 class="h-description flame-text">
	...
	created() { //生命周期函数
		...
		var onVisibilityChange = function () {
			if (document[hiddenProperty]) { //被隐藏
				...
				document.title = '聚隆博客 被发现啦(*´∇｀*)'; //当前窗口打开
...
.flame-text .text-content {
	background-image: linear-gradient(to right, black, #ff5c00, #ffdb00, #ff5c00, black);
	-webkit-background-clip: text;
	background-clip: text;
	color: transparent;
	animation: burn 2s linear infinite, flicker 0.15s linear infinite;
}
...
@keyframes burn {
	...
	50% {
		text-shadow: 0px 0px 20px rgba(255, 255, 255, 1), 0px 0px 40px rgba(255, 255, 255, 1);
	}
...
@keyframes flicker {
	...
	0%,
	100% {
		text-shadow: 0px 0px 40px rgba(255, 255, 255, 1), 0px 0px 60px rgba(255, 255, 255, 1), 0px 0px 80px rgba(255, 255, 255, 1);
	}
...
.headImgBox {
	...
	background-repeat: no-repeat;
	margin-bottom: 90px;
	background: url(../../static/img/jay2.mp4) no-repeat center 50%;
	z-index: 1;
	/* 将 z-index 值调整为比 .h-information 的 z-index 值更低 */
}

.headImgBox .video-background {
	position: absolute;
	top: 0;
	left: 0;
	width: 100%;
